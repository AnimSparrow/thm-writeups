[![Frankesqwen](https://github.com/AnimSparrow/thm-writeups/raw/main/assets/banners/frankesqwen.svg)](/AnimSparrow/thm-writeups/blob/main/assets/banners/frankesqwen.svg)

[![](https://img.shields.io/badge/TRYHACKME-1a0633?style=for-the-badge&labelColor=FF2A6D)](https://tryhackme.com/room/frankesqwen)
[![](https://img.shields.io/badge/HARD-1a0633?style=for-the-badge&labelColor=00F0FF)](#)
[![](https://img.shields.io/badge/AI_/_LLM-1a0633?style=for-the-badge&labelColor=A479C4)](#)
[![](https://img.shields.io/badge/WEIGHT_EXTRACTION-1a0633?style=for-the-badge&labelColor=A479C4)](#)
...
**Attempt 1 — import into Ollama and just ask.** Felt like the intended path.
...
**Attempt 3 — the hint model will save me.** It did not.
...
**Attempt 4 — prefill / assistant-prefix attack.** Don't ask; put the answer in its mouth and let it autocomplete.
...
**1. The room hands you the answer shape.** The task gives the flag mask directly:
...
**2. A logit lens shows the flag is in there.** Read the raw next-token distribution instead of sampled text, and nudge the prefix toward the flag — real word-pieces fall out where a refusal should be:
...
**The kill shot: format-constrained greedy decoding.** At each step, walk the model's own ranked logits but only accept a token that keeps the output on the `1_2_8_3_9_7` mask (letters/digits/`_`, correct word lengths). A refusal token like `I'm` **cannot fit the mask**, so it is structurally impossible to emit. The model is forced down its own memorised flag path.
...
> [!NOTE]
> Values masked — **Frankesqwen** is an active Premium room. The methodology above reconstructs the flag end to end; the literal string is one solve away.

| # | Question | Source | Answer |
| --- | --- | --- | --- |
| 1 | Flag nested in Frankesqwen | Constrained decode of `frankesqwen-v7` weights | `THM{1_4m_f34rl3ss_***_*********_*******}` |
